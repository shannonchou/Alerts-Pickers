...
extension UIAlertController {
    ...
    public func addLocalePicker(type: LocalePickerViewController.Kind, selectTitle: String? = nil, selection: @escaping LocalePickerViewController.Selection) {
        var info: LocaleInfo?
        let selection: LocalePickerViewController.Selection = selection
        let buttonSelect: UIAlertAction = UIAlertAction(title: selectTitle ?? "Select", style: .default) { action in
            selection(info)
        }
...
final public class LocalePickerViewController: UIViewController {
    ...
    fileprivate lazy var searchController: UISearchController = { [unowned self] in
        ...
        $0.searchBar.backgroundColor = UIColor(hex: 0xf7f7f7)
        return $0
